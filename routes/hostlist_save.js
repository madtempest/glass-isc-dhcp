...
router.post('/', authorize.auth, function(req, res, next) {
	...
	//fs.writeFileSync("./syntax_verify_config", request.hostlist_data, 'utf8');

	var exec = require('child_process').exec;

	exec('/usr/sbin/dhcpd -t -cf /etc/dhcp/dhcpd.conf > verify_output 2> verify_output', function(err, stdout, stderr)
	{
		...
		if (err) {
			...
			//date +"%Y-%m-%d_%H:%M:%S"
			exec('/bin/cp ' + glass_config.hostlist_file + ' ./config_backups/`basename ' + glass_config.hostlist_file + '`_`date +"%Y-%m-%d_%H:%M:%S"`',
				function(err, stdout, stderr) {

			});

			fs.writeFileSync(glass_config.hostlist_file, request.hostlist_data, 'utf8');
