...
router.post('/', authorize.auth, function(req, res, next) {
	...
	//fs.writeFileSync("./syntax_verify_config", request.subnet_data, 'utf8');
	...
	exec('/usr/sbin/dhcpd -t -cf /etc/dhcp/dhcpd.conf > verify_output 2> verify_output', function(err, stdout, stderr)
	{
		...
		if (err) {
			...
			//date +"%Y-%m-%d_%H:%M:%S"
			exec('/bin/cp ' + glass_config.subnet_file + ' ./config_backups/`basename ' + glass_config.subnet_file + '`_`date +"%Y-%m-%d_%H:%M:%S"`',
				function(err, stdout, stderr) {

			});

			fs.writeFileSync(glass_config.subnet_file, request.subnet_data, 'utf8');
